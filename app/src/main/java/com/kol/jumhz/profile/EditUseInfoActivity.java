...
/**
 * @ClassName: EditUseInfoActivity
 * @Author: LanLnk
 * @CreateDate: 2020-05-06 15:01
 * @Description: 用户资料编辑页面
 */
public class EditUseInfoActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        mEditView.setText(UserMgr.getInstance().getNickname());
        Utils.showPicWithUrl(this, mIvAvatar, UserMgr.getInstance().getAvatar(), R.drawable.ic_camera_download_bg);
        path.clear();
        path.add(0, UserMgr.getInstance().getAvatar());
        mTvTitle.setReturnListener(v -> {
            if ("".equals(path.get(0))) {
                TipDialog.show(EditUseInfoActivity.this, "请设置头像", TipDialog.TYPE.WARNING);
                return;
            }
            finish();
        });
        mTvTitle.setMoreListener(v -> {
            ...
            WaitDialog.show(EditUseInfoActivity.this, "");
            if (!path.get(0).contains("http") && !"".equals(path.get(0))) {
                UserMgr.getInstance().uploadImage(path.get(0), new HTTPMgr.Callback() {
                    @Override
                    public void onSuccess(JSONObject data) {
                        UserMgr.getInstance().uploadUserInfo(Objects.requireNonNull(data.optJSONObject("result")).optString("pic_url"),
                                mEditView.getText().toString(), new HTTPMgr.Callback() {
                                    ...
                                    public void onFailure(int code, String msg) {
                                        ...
                                        mainHandler.post(() -> TipDialog.show(EditUseInfoActivity.this, msg+":"+code, TipDialog.TYPE.ERROR)); }
                                });
                    }
                    @Override
                    public void onFailure(int code, String msg) {
                ...
                if ("".equals(path.get(0))) {
                    TipDialog.show(EditUseInfoActivity.this, "请选择头像", TipDialog.TYPE.WARNING);
                    return;
                }
                UserMgr.getInstance().uploadUserInfo(UserMgr.getInstance().getAvatar(),
                        mEditView.getText().toString(), new HTTPMgr.Callback() {
                            @Override
                            public void onSuccess(JSONObject data) {
                                WaitDialog.dismiss();
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.rl_eui_head) {
            ...
            GalleryPick.getInstance().setGalleryConfig(Utils.galleryConfig(iHandlerCallBack)).open(EditUseInfoActivity.this);
        }
    }
