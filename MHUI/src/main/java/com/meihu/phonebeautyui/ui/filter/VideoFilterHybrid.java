 ...
 */
public class VideoFilterHybrid extends ZegoVideoFilter {
    ...
    protected void allocateAndStart(Client client) {
        Log.d("meihu_sdk_VideoFilter","------allocateAndStart");
        mClient = client;
    ...
    protected void stopAndDeAllocate() {
        Log.d("meihu_sdk_VideoFilter","------stopAndDeAllocate");
        final CountDownLatch barrier = new CountDownLatch(1);
...
//                int textureID = mhBeautyManager.renderDualRGBA(mModiBuffer,mTextureId, pixelBuffer.width, pixelBuffer.height,0);
                // 此步骤会使用此 textureID 用做本地预览的视图渲染，并将美颜后的 textureID 传给 SDK（拉该条流时是美颜后的视频）
                int textureID = mTextureId;
                textureID = mhBeautyManager.render(mTextureId, pixelBuffer.width, pixelBuffer.height);
