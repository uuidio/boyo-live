...
public class StickerPagerAdapter extends PagerAdapter {
    ...
    public StickerPagerAdapter(List<StickerCategaryBean> titles) {
        this.titles.clear();
...
//        int i = viewsList.indexOfKey(position);
        ...
        StickerAdapter stickerAdapter = new StickerAdapter(container.getContext(), titles.get(position).getId());
...
//                    effectUpdateListener.onBrightChanged(Integer.valueOf(stickerBeautyData.getSkin_brightness()));
                    ...
                    if (lastNormalBeautyDataMap == null || lastNormalBeautyDataMap.size() <= 0) return;
