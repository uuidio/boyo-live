...
//        mTextView = (TextView) v.findViewById(R.id.text);
        ...
        mProgressVal.setVisibility(INVISIBLE);
...
//        mProgressVal.setX(pox);
        post(() -> {
            float mProgressValWidth = mProgressVal.getMeasuredWidth();
            float measuredWidth = mSeekBar.getMeasuredWidth();
            if (mProgressValWidth == 0 || measuredWidth == 0) return;
...
//        int realWidth = measuredWidth - DensityUtils.dp2px(mContext, 40);
            ...
            mProgressVal.setVisibility(VISIBLE);
        });
    }
